...
- `lecture1/mindbody.png` — в раздел «VIII. Мозг и нервная система»
- `lecture1/chakraflows.jpg` — в раздел «X. Система циркуляции чакры»

### ⚠️ Правила для фотографий

1. **Имена файлов** — используйте snake_case (например `chakraflows.jpg`, `mindbody.png`)
2. **Формат** — `.jpg`, `.png` или `.webp`
3. **Размер** — любые размеры, CSS сам масштабирует через `object-fit` и адаптивную сетку
4. **Alt-текст** — всегда заполняйте `alt="Описание"` для доступности
5. **Папка** — файл должен лежать в папке именно той лекции, где используется (не в `images/`)
